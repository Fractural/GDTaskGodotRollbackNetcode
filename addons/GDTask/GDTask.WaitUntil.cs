...
namespace Fractural.Tasks
{
    ...
    {
        public static GDTask WaitUntil(Func<bool> predicate, IPlayerLoopTiming timing = null, CancellationToken cancellationToken = default(CancellationToken))
        {
            if (timing == null) timing = ProcessLoopTiming.Type;
            return new GDTask(WaitUntilPromise.Create(predicate, timing, cancellationToken, out var token), token);
        }

        public static GDTask WaitWhile(Func<bool> predicate, IPlayerLoopTiming timing = null, CancellationToken cancellationToken = default(CancellationToken))
        {
            if (timing == null) timing = ProcessLoopTiming.Type;
            return new GDTask(WaitWhilePromise.Create(predicate, timing, cancellationToken, out var token), token);
        }

        public static GDTask WaitUntilCanceled(CancellationToken cancellationToken, IPlayerLoopTiming timing = null)
        {
            if (timing == null) timing = ProcessLoopTiming.Type;
            return new GDTask(WaitUntilCanceledPromise.Create(cancellationToken, timing, out var token), token);
        }

        public static GDTask<U> WaitUntilValueChanged<T, U>(T target, Func<T, U> monitorFunction, IPlayerLoopTiming monitorTiming = null, IEqualityComparer<U> equalityComparer = null, CancellationToken cancellationToken = default(CancellationToken))
          where T : class
        {
            if (monitorTiming == null) monitorTiming = ProcessLoopTiming.Type;
        ...
        sealed class WaitUntilPromise : IGDTaskSource, IPlayerLoopItem, ITaskPoolNode<WaitUntilPromise>
        {
            ...
            public static IGDTaskSource Create(Func<bool> predicate, IPlayerLoopTiming timing, CancellationToken cancellationToken, out short token)
            {
        ...
        sealed class WaitUntilCanceledPromise : IGDTaskSource, IPlayerLoopItem, ITaskPoolNode<WaitUntilCanceledPromise>
        {
            ...
            public static IGDTaskSource Create(CancellationToken cancellationToken, IPlayerLoopTiming timing, out short token)
            {
                if (cancellationToken.IsCancellationRequested)
                {
                    return AutoResetGDTaskCompletionSource.CreateFromCanceled(cancellationToken, out token);
        ...
        sealed class WaitUntilValueChangedGodotObjectPromise<T, U> : IGDTaskSource<U>, IPlayerLoopItem, ITaskPoolNode<WaitUntilValueChangedGodotObjectPromise<T, U>>
        {
            ...
            public static IGDTaskSource<U> Create(T target, Func<T, U> monitorFunction, IEqualityComparer<U> equalityComparer, IPlayerLoopTiming timing, CancellationToken cancellationToken, out short token)
            {
                if (cancellationToken.IsCancellationRequested)
                {
